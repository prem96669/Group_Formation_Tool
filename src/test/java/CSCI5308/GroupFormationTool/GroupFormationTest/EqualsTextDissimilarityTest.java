...
class EqualsTextDissimilarityTest {
	
	ITextComparison itc = new EqualsTextDissimilarity();
	...
	void testGetScore() {
		...
		Assert.isTrue(itc.getScore(choice1, choice2) == 0.0);
		
		// Test 2: Returned value must be 0.0, i.e., similar(ignore case)
		Assert.isTrue(itc.getScore(choice1, choice3) == 0.0);
				
		// Test 3: Returned value must be 1.0, i.e., dissimilar
		Assert.isTrue(itc.getScore(choice1, choice4) == 1.0);
	}
